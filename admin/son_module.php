...
<div id="main">
	...
	<table class="list">
		...
		<?php 
		// $query="select ssm.id,ssm.sort,ssm.module_name,
		// sfm.module_name father_module_name,ssm.member_id 
		// from son_module ssm,father_module sfm 
		// where ssm.father_module_id=sfm.id 
		// order by sfm.id";
		$query="select ssm.id,ssm.sort,ssm.module_name,
		sfm.module_name father_module_name,sm.name name
		from son_module ssm,father_module sfm, member sm 
		where ssm.father_module_id=sfm.id 
		and ssm.member_id=sm.id
		order by sfm.id";
