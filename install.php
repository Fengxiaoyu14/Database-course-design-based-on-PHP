...
if(isset($_POST['submit'])){
	...
	if(mysqli_num_rows($result)!=1){
		$query_info_i="INSERT INTO `info` (`id`, `title`, `keywords`, `description`) VALUES(1, 'test', 'test', 'test');";
		mysqli_query($link,$query_info_i);
		if(mysqli_errno($link)){
			exit("数据库info写入数据失败请检查相应权限!<a href='install.php'>点击返回</a>");
		}
	}
	$query_manage_s="select * from manage where name='admin'";
	$result=mysqli_query($link, $query_manage_s);
	if(mysqli_num_rows($result)!=1){
		$query_manage_i="INSERT INTO `manage` (`name`, `pw`, `create_time`, `level`) VALUES('admin',md5('{$_POST['manage_pw']}'),now(), 0)";
		mysqli_query($link,$query_manage_i);
		if(mysqli_errno($link)){
			exit("管理员创建失败，请检查数据表manage是否具有写权限!<a href='install.php'>点击返回</a>");
		}
	}
